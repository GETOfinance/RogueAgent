...
-- 0G Storage references: links Supabase records to decentralized 0G Storage
CREATE TABLE public.zg_storage_refs (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  root_hash text NOT NULL UNIQUE,
  category text NOT NULL CHECK (category = ANY (ARRAY['chart'::text, 'dataset'::text, 'model_output'::text, 'agent_memory'::text, 'backtest'::text, 'report'::text, 'encrypted'::text])),
  file_key text,
  ...
  metadata jsonb,
  created_at timestamp with time zone NOT NULL DEFAULT now(),
  CONSTRAINT zg_storage_refs_pkey PRIMARY KEY (id)
);

-- 0G Fine-tuning tasks
CREATE TABLE public.zg_finetuning_tasks (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  task_id text NOT NULL UNIQUE,
  provider_address text NOT NULL,
  model text NOT NULL,
  dataset_root_hash text,
  config jsonb,
  status text NOT NULL DEFAULT 'Init' CHECK (status = ANY (ARRAY['Init'::text, 'SettingUp'::text, 'SetUp'::text, 'Training'::text, 'Trained'::text, 'Delivering'::text, 'Delivered'::text, 'UserAcknowledged'::text, 'Finished'::text, 'Failed'::text])),
  fee numeric,
  encrypted_model_path text,
  decrypted_model_path text,
  created_at timestamp with time zone NOT NULL DEFAULT now(),
  updated_at timestamp with time zone NOT NULL DEFAULT now(),
  CONSTRAINT zg_finetuning_tasks_pkey PRIMARY KEY (id)
);

-- 0G Inference usage tracking
CREATE TABLE public.zg_inference_usage (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  provider_address text NOT NULL,
  model text NOT NULL,
  ...
  agent_type text,
  created_at timestamp with time zone NOT NULL DEFAULT now(),
  CONSTRAINT zg_inference_usage_pkey PRIMARY KEY (id)
);

-- Add 0G chain support to users table
ALTER TABLE public.users ADD COLUMN IF NOT EXISTS rge_balance_0g_testnet numeric;
ALTER TABLE public.users ADD COLUMN IF NOT EXISTS rge_balance_0g_mainnet numeric;
ALTER TABLE public.users ADD COLUMN IF NOT EXISTS preferred_chain text DEFAULT 'fraxtal';